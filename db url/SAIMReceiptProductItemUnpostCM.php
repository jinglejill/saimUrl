<?php
    ...
    if (isset ($_POST["countProduct"]))
    {
        ...
        for($i=0; $i<$countProduct; $i++)
        {
            ...
            $customMadeIDEdit[$i] = $_POST["customMadeIDEdit".sprintf("%02d", $i)];
    ...
    
    {
        ...
        {
            //query statement
            $sql = "update product set Status = 'I' where ProductID = $productID[$i]";
        ...
        {
            //query statement
            $sql = "update receiptproductitem set ProductType = 'C', ProductID = '$customMadeIDEdit[$i]' where ReceiptProductItemID = $receiptProductItemID[$i]";
            $ret = doQueryTask($con,$sql,$_POST["modifiedUser"]);
            if($ret != "")
            {
                putAlertToDevice($_POST["modifiedUser"]);
