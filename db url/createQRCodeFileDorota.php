...
<body>
<form id="myForm" action="createQRCodeFileDorota.php" method="post">
<?php
    ...
    $arrProductCode = [];
    $arrProductName = [];
    $sql2 = "SELECT * from (SELECT concat(productname.ProductCategory2,productname.ProductCategory1,productname.Code) as Cat2Cat1ProductName, productname.Name AS ProductName FROM `productname`) Cat2Cat1ProductName ORDER BY Cat2Cat1ProductName.ProductName";
    ...
    //price
    $arrProductPrice = [];
    $sql2 = "SELECT productname.Name as ProductName,color.Name as Color,productsize.SizeLabel, productsales.Price from productsales left join productname on productsales.ProductNameID=productname.ProductNameID LEFT JOIN color ON productsales.Color=color.Code LEFT JOIN productsize on productsales.Size = productsize.Code where productsalessetid = 0 ORDER BY productname.Name, color.Name, productsize.SizeLabel";
    $result2 = mysqli_query($con2, $sql2);
    while ($row2 = mysqli_fetch_array($result2)) {
...
//                }
    ...
    if($validate == 1 && $_POST["create"])
    {
        ...
        for($k=1; $k<=$rowCount; $k++)
        {
            ...
            {
                $qrCodeFormat = "SAIM Dorota\n%s\nEnd";
                $code = $_POST["productName".$r02].$_POST["color".$r02.$_POST["productName".$r02]].str_pad($_POST["size".$r02.$_POST["productName".$r02]],2,'0',STR_PAD_LEFT) . str_replace('-','',$_POST["manufacturingDate".$r02]) . sprintf("%06d", $maxProductID + $i);
        ...
        if(result2)
        {
            ...
            $objPHPExcel->getProperties()->setLastModifiedBy("Dorota");
