<?php    
    ...
    $sql = "INSERT INTO `productionorder`(`ProductionOrderID`,`RunningPoNo`, `ProductNameID`, `Color`, `Size`, `Quantity`, `Status`, `OrderDeliverDate`) values($startProductionOrderID,$runningPoNo,$productNameID[0],'$color[0]','$size[0]',1,-1,now())";
    for($i=1; $i<$countProduct; $i++)
    {
        //query statement
        $sql .= ",($startProductionOrderID+$i,$runningPoNo,$productNameID[$i],'$color[$i]','$size[$i]',1,-1,now())";
    }
    ...
    //ไม่ได้ทำ pushsynce เพราะทุกครั้งที่ใช้จะโหลดจาก db ขึ้นมาใหม่ตลอด ใน program ไม่มี sharedProductionOrder
